...
| [`scripts/`](scripts/)                                                         | **operation**        | Everything needed to publish and serve: `deploy-frontend.sh` (staged, atomic publish, with a confirmation gate), `build-frontend-public.sh`, `build-packs.sh` (vectorize). Call order in [`scripts/README.md`](scripts/README.md); the API itself starts from `start_server.sh` at the root. | Bash + Python                |
