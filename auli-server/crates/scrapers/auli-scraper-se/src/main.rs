// auli-scraper-se — scraper da SEFAZ-SE (serviços, www.sefaz.se.gov.br — SharePoint 2013).
// Sem headless: a Carta de Serviços é uma única página (servicos_cidadao.aspx) com 91 painéis.
//
// Conhece UMA entidade ("se"); não lê o registry. Grava o snapshot de serviços (v3); a derivação
// dos artefatos é o `auli-collections se`.

mod se;

/// A entidade que este scraper conhece (um crate binário por entidade).
pub const ENTITY: &str = "se";
const DATA_DIR: &str = "../data/se/raw";

fn main() -> Result<(), Box<dyn std::error::Error>> {
    // CLI: auli-scraper-se [--usecache] servicos   (omitido -> servicos)
    ...
    println!("🏛️  Scraper SE (SEFAZ-SE) — coleção: {}", cmd);
    if use_cache {
...
fn run_servicos(use_cache: bool) -> Result<(), Box<dyn std::error::Error>> {
    // Montagem direta de `ServicoRaw` (padrão AC/PB): identidade = a âncora do painel (link).
    let (items, publicos_ordem) = se::scrape(DATA_DIR, use_cache)?;
