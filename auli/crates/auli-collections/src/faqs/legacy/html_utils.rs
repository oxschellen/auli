...
/// Finds the element whose opening tag contains `class="<class_attr>"`, then walks
/// forward counting open/close `tag_name` tags to locate the matching closing tag,
/// and returns the inner HTML slice.
...
pub fn html_to_text_with_links(html: &str) -> String {
...
pub(crate) fn extract_question_title_from_class(html_chunk: &str, class_attr: &str) -> String {
...
pub(crate) fn extract_question_href_from_class(html_chunk: &str, class_attr: &str) -> String {
    if let Some(class_pos) = html_chunk.find(class_attr) {
