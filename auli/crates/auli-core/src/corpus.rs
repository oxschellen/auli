...
//
// This is the module that MUST agree between `update` (embeds documents) and `server` (embeds
// the query): if `parse_*`/`prepare_documents` change, the on-disk vectors and a live query stop
// describing the same thing. Bump `manifest::STRATEGY_VERSION` whenever anything here changes.
...
// All four kinds, for callers that iterate (e.g. `auli update` ingesting an entity end-to-end).
...
pub fn parse_block_lines(lines: Vec<String>, delimiter: &str) -> Vec<String> {
...
mod tests {
    ...
    #[test]
    fn qa_mode_skips_comments_and_gates_on_first_delimiter() {
        let text = "// preamble to ignore\n## pergunta\nQ1\n## resposta\nA1\n## pergunta\nQ2\n## resposta\nA2\n";
        let blocks = parse_blocks_from_text(text, "## pergunta");
        assert_eq!(blocks.len(), 2);
        assert!(blocks[0].starts_with("## pergunta\nQ1"));
        assert!(blocks[1].contains("Q2"));
    }

    #[test]
    fn servicos_mode_keeps_delimiter_lines_and_collects_from_start() {
        let text = "// 1.\nTitulo A\n// 2.\nTitulo B\n";
        let blocks = parse_blocks_from_text(text, "//");
        assert_eq!(blocks.len(), 2);
        assert!(blocks[0].starts_with("// 1."));
    }

    #[test]
    fn question_key_embeds_pergunta_only_but_stores_full_block() {
        let blocks = vec!["## pergunta\nComo emitir nota?\n## resposta\nVá ao portal.".to_string()];
        let (stored, to_embed) = prepare_documents(&blocks, &FAQS);
        assert_eq!(to_embed[0], "Como emitir nota?");
        assert!(stored[0].contains("## resposta"));
    }

    #[test]
    fn description_embeds_first_lines_stores_cleaned() {
        let blocks = vec!["// 1.\nLinha1\nLinha2\nLinha3\nLinha4\nLinha5\nAcessar o serviço".to_string()];
        let (stored, to_embed) = prepare_documents(&blocks, &SERVICES);
        // embed = first 4 non-empty, non-`//` lines
        assert_eq!(to_embed[0], "Linha1\nLinha2\nLinha3\nLinha4");
        // stored drops `//` and "Acessar o serviço" lines
        assert!(!stored[0].contains("Acessar o serviço"));
        assert!(!stored[0].contains("// 1."));
    }

    #[test]
    fn full_text_embeds_and_stores_identical() {
        let blocks = vec!["## pergunta\nbloco unico".to_string()];
        let (stored, to_embed) = prepare_documents(&blocks, &NOTAS);
        assert_eq!(stored, to_embed);
    }
}
